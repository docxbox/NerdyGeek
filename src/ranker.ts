import type { RankedChunk, RetrievedDocument, SourceType } from "./types.js";
...
export function rankChunks(documents: RetrievedDocument[], query: string): RankedChunk[] {
  const keywords = queryTerms(query);
  const normalizedQuery = query.toLowerCase().replace(/\(\)/g, "");

  return documents
    .flatMap((document) =>
      extract(document.html).map((text) => {
        ...
        const urlOverlap = keywords.reduce(
          (count, keyword) => count + (normalizedUrl.includes(keyword) ? 1 : 0),
          0
        );
