...
function scoreCodeBlock(code: string, query: string, url: string): number {
...
function chooseBestCode(documents: RetrievedDocument[], rankedChunks: RankedChunk[], query: string): string | undefined {
  ...
  const candidates = orderedDocuments.flatMap((document) =>
    extractRelevantCodeBlocks(document.html).map((code) => ({
      code,
