...
function compactList(items, mode) {
    return unique(items.map((item) => normalizeText(item)).filter((item) => item.length >= 8)).slice(0, maxListByMode[mode]);
}
function collectGotchasFromChunks(chunks, mode) {
    return compactList(chunks
        .map((chunk) => chunk.text)
        .filter((text) => /\b(deprecated|warning|requires|must|only|cannot|avoid|breaking)\b/i.test(text)), mode);
}
function collectActionsFromChunks(chunks, mode) {
    return compactList(chunks
        .map((chunk) => chunk.text)
        .filter((text) => /\b(use|call|set|pass|import|configure|migrate|replace)\b/i.test(text)), mode);
}
function summarizeChunkTexts(chunks, mode) {
...
function groupChangeEntries(changes, type, mode) {
    return compactList(changes
...
export function formatDiffDocsEnvelope(input) {
    ...
    const actions = compactList(input.changes
        .filter((change) => change.replacement || change.type === "breaking")
        .map((change) => change.replacement
        ? `${change.description} Replace with ${change.replacement}.`
        : change.description), mode);
    const gotchas = compactList(input.changes
        .filter((change) => change.type === "removed" || change.type === "breaking")
        .map((change) => change.description), mode);
    const summary = `${input.stack} ${input.fromVersion} -> ${input.toVersion}: ` +
    ...
    const orderedChanges = [
        ...groupChangeEntries(input.changes, "breaking", mode).map((description) => ({ type: "breaking", description })),
        ...groupChangeEntries(input.changes, "removed", mode).map((description) => ({ type: "removed", description })),
        ...groupChangeEntries(input.changes, "deprecated", mode).map((description) => ({ type: "deprecated", description })),
        ...groupChangeEntries(input.changes, "new", mode).map((description) => ({ type: "new", description }))
    ];
    return {
        tool: "diff_docs",
        stack: input.stack,
...
function formatDeprecationAction(match) {
...
export function formatScanEnvelope(input) {
    const mode = input.mode ?? "full";
    const summary = input.deprecated.length === 0 && input.removed.length === 0
    ...
    return {
        ...
        mode,
        summary,
        actions: compactList([...input.removed, ...input.deprecated].map(formatDeprecationAction), mode),
        gotchas: compactList([...input.removed, ...input.deprecated].map((match) => match.reason), mode),
        sources: input.sources,
