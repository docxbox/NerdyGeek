...
function scoreCandidateLink(query, href, anchorText) {
...
async function discoverRelatedUrls(baseUrl, query) {
...
export async function retrieveDocs(urls, query) {
    const expanded = unique((await Promise.all(urls.map(async (url) => {
        try {
    ...
    }))).flat()).slice(0, config.maxRetrievedPages);
    const documents = await Promise.all(expanded.map(async (url) => {
