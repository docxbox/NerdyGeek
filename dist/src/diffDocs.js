...
function headingType(heading) {
    ...
    if (/(removed|removal)/.test(lower))
        return "removed";
    if (/(deprecat|sunsetting)/.test(lower))
        return "deprecated";
    if (/(breaking|behavior|behaviour|changed|changes)/.test(lower))
        return "breaking";
    if (/(new|added|introduced|feature)/.test(lower))
        return "new";
    return null;
}
