...
async function main() {
    ...
    assert.equal(resolveVersion("nextjs", "next 14.2.1 server actions docs", {
        dependencies: {
            next: "^13.5.0"
        }
    }), "14");
...
export async function create() {
  ...
  `);
    ...
    assert.equal(resolveVersion("go", "net/http routing", undefined, { goMod: "module myapp\n\ngo 1.22\n" }), "1");
    assert.equal(resolveVersion("tokio", "tokio async", undefined, {
        cargoToml: '[dependencies]\ntokio = { version = "1.28.0", features = ["full"] }\n'
    }), "1");
    assert.equal(resolveVersion("flask", "flask route", undefined, {
        requirementsTxt: "Flask==3.0.2\nrequests>=2.31.0\n"
    }), "3");
    assert.equal(resolveVersion("rails", "activerecord", undefined, {
        gemfileLock: "GEM\n  specs:\n    rails (7.1.3)\n    rake (13.0.6)\n"
    }), "7");
    // --- classifyChunk ---
